...
"""
function load_generators_data!(setup::Dict, path::AbstractString, inputs_gen::Dict, source_flag::Bool, number_of_scenarios::Int64)
...
# Dharik - Done, we have scaled fuel costs above so any parameters on per MMBtu do not need to be scaled
	...
	if source_flag
		inputs_gen["C_Fuel_per_MWh"] = zeros(Float64, G, inputs_gen["T_scenario_1"], number_of_scenarios)
	else
	...
	if source_flag
		for sc in 1:number_of_scenarios
			...
			for g in 1:G
				...
				inputs_gen["C_Fuel_per_MWh"][g,:,sc] = fuel_costs[fuel_type[g]].*heat_rate[g]
