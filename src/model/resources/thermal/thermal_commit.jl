...
function thermal_commit!(EP::Model, inputs::Dict, setup::Dict)
	...
	@constraints(EP, begin
		...
		[y in THERM_COMMIT, t in START_SUBPERIODS, sc=1:SC], EP[:vCOMMIT][y,t,sc] == EP[:vCOMMIT][y,(t+hours_per_subperiod-1),sc] + EP[:vSTART][y,t,sc] - EP[:vSHUT][y,t,sc]
		...
		[y in THERM_COMMIT, t in INTERIOR_SUBPERIODS, sc=1:SC], EP[:vCOMMIT][y,t,sc] == EP[:vCOMMIT][y,t-1,sc] + EP[:vSTART][y,t,sc] - EP[:vSHUT][y,t,sc]
	end)
	...
	# Links last time step with first time step, ensuring position in hour 1 is within eligible ramp of final hour position
		# rampup constraints
	@constraint(EP,[y in THERM_COMMIT, t in START_SUBPERIODS, sc=1:SC],
		EP[:vP][y,t,sc]-EP[:vP][y,(t+hours_per_subperiod-1),sc] <= dfGen[y,:Ramp_Up_Percentage]*dfGen[y,:Cap_Size]*(EP[:vCOMMIT][y,t,sc]-EP[:vSTART][y,t,sc])
	...
	@constraint(EP,[y in THERM_COMMIT, t in START_SUBPERIODS, sc=1:SC],
		EP[:vP][y,(t+hours_per_subperiod-1),sc]-EP[:vP][y,t,sc] <= dfGen[y,:Ramp_Dn_Percentage]*dfGen[y,:Cap_Size]*(EP[:vCOMMIT][y,t,sc]-EP[:vSTART][y,t,sc])
	...
	## For Interior Hours
		# rampup constraints
	@constraint(EP,[y in THERM_COMMIT, t in INTERIOR_SUBPERIODS, sc=1:SC],
		EP[:vP][y,t,sc]-EP[:vP][y,t-1,sc] <= dfGen[y,:Ramp_Up_Percentage]*dfGen[y,:Cap_Size]*(EP[:vCOMMIT][y,t,sc]-EP[:vSTART][y,t,sc])
			+ min(inputs["pP_Max"][y,t,sc],max(dfGen[y,:Min_Power],dfGen[y,:Ramp_Up_Percentage]))*dfGen[y,:Cap_Size]*EP[:vSTART][y,t,sc]
			-dfGen[y,:Min_Power]*dfGen[y,:Cap_Size]*EP[:vSHUT][y,t,sc])

		# rampdown constraints
	@constraint(EP,[y in THERM_COMMIT, t in INTERIOR_SUBPERIODS, sc=1:SC],
		EP[:vP][y,t-1,sc]-EP[:vP][y,t,sc] <= dfGen[y,:Ramp_Dn_Percentage]*dfGen[y,:Cap_Size]*(EP[:vCOMMIT][y,t,sc]-EP[:vSTART][y,t,sc])
			-dfGen[y,:Min_Power]*dfGen[y,:Cap_Size]*EP[:vSTART][y,t,sc]
			+min(inputs["pP_Max"][y,t,sc],max(dfGen[y,:Min_Power],dfGen[y,:Ramp_Dn_Percentage]))*dfGen[y,:Cap_Size]*EP[:vSHUT][y,t,sc])
