...
function must_run!(EP::Model, inputs::Dict, setup::Dict)
	...
	@expression(EP, ePowerBalanceNdisp[t=1:T, z=1:Z, sc=1:SC],
		sum(EP[:vP][y,t,sc] for y in intersect(MUST_RUN, dfGen[dfGen[!,:Zone].==z, :R_ID])))

	EP[:ePowerBalance] += ePowerBalanceNdisp

	# Capacity Reserves Margin policy
	if CapacityReserveMargin > 0
		@expression(EP, eCapResMarBalanceMustRun[res=1:inputs["NCapacityReserveMargin"], t=1:T, sc=1:SC], sum(dfGen[y,Symbol("CapRes_$res")] * EP[:eTotalCap][y] * inputs["pP_Max"][y,t,sc]  for y in MUST_RUN))
		EP[:eCapResMarBalance] += eCapResMarBalanceMustRun
	end

	### Constratints ###

	@constraint(EP, [y in MUST_RUN, t=1:T, sc=1:SC], EP[:vP][y,t,sc] == inputs["pP_Max"][y,t,sc]*EP[:eTotalCap][y])
	##CO2 Polcy Module Must Run Generation by zone
	@expression(EP, eGenerationByMustRun[z=1:Z, t=1:T, sc=1:SC], # the unit is GW
		sum(EP[:vP][y,t,sc] for y in intersect(MUST_RUN, dfGen[dfGen[!,:Zone].==z, :R_ID]))
	)
